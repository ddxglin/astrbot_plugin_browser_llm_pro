...
# 搜索引擎名 → 起始页（面板里的 default_search_engine 是自由文本，这里只认常见的几个）
_ENGINE_HOME = {
...
class BrowserLLMPluginStar(Star):
    ...
    def _convert_config(self, config: AstrBotConfig) -> dict:
        ...
        plugin_config = {
            ...
            "flash_ruffle": config.get("flash_ruffle", True),
            "browser_proxy": (config.get("browser_proxy") or "").strip(),
            "mem_min_available_mb": config.get("mem_min_available_mb", 900),
            "default_search_engine": config.get("default_search_engine", "必应搜索"),
            "banned_words": config.get("banned_words", []),
            # 起始页：面板没填就按搜索引擎决定（以前这里回落到百度，导致"设了必应还是打开百度"）
            "default_url": (config.get("default_url") or "").strip()
            or _ENGINE_HOME.get(config.get("default_search_engine", ""), "https://www.bing.com"),
            "viewport_size": {
            ...
            # 动作类工具（点击/按键/滚动等）是否把刚截的图直接带回给模型：省一轮往返
            "action_screenshot": config.get("action_screenshot", True),
            # 看图默认宽度：越小 token 越少、模型越快（实测 1920→800 省约 0.9s/次）
            "screenshot_max_width": config.get("screenshot_max_width", 1024),
            # 动作结果里附一行页面摘要（标题/URL/可点元素/正文开头）：省掉"单独打听"的往返
            "action_digest": config.get("action_digest", True),
            "action_digest_chars": config.get("action_digest_chars", 120),
            # 慢站点：打开网页与单元素动作的等待上限（原来是 30s 卡死）
            "nav_timeout": config.get("nav_timeout", 15),
            "action_timeout_ms": config.get("action_timeout_ms", 3000),
            ...
            # ===== 录屏 =====
            "record_max_duration": config.get("record_max_duration", 30),
            "record_fps": config.get("record_fps", 15),
            "record_quality": config.get("record_quality", 70),
            "record_max_width": config.get("record_max_width", 0),
            "record_max_height": config.get("record_max_height", 0),
            "record_crf": config.get("record_crf", 30),
            "record_keep_files": config.get("record_keep_files", 20),
            # 录屏是否连声音一起录（依赖容器内 PulseAudio 虚拟声卡，见 core/audio.py）
            "record_audio": config.get("record_audio", True),
            "supervisor": {
                # 主判据：整机内存百分比（psutil 读的是宿主机水位——Docker 不隔离 /proc/meminfo）
                "max_memory_percent": config.get("max_memory_percent", 80),
                # 次级判据：AstrBot 自身进程树预算（≤0 关闭这条）
                "astrbot_memory_budget_mb": config.get("astrbot_memory_budget_mb", 3072),
